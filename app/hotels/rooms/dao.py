...
class RoomsDAO(BaseDAO):
    ...
    async def get_rooms_list(cls, hotel_id, date_from: date, date_to: date):
        ...
        SELECT
            ...
            quantity,
            image_id,
            (price * 25) AS total_cost,
            (quantity - COALESCE(booked_rooms, 0)) AS rooms_left
        FROM rooms_query
        ...
        async with async_session_maker() as session:
            ...
            booked_rooms = (
                select(
                    Bookings.room_id, func.count(Bookings.room_id).label("booked_rooms")
                )
            ...
            data = (
                ...
                .join(
                    booked_rooms,
                    rooms_query.c.id == booked_rooms.c.room_id,
                    isouter=True,
                )
